...
for j=1:12
    ...
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0068',0.0628*v1(j),'b');%acetate
    ...
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0002',1.8729*v1(j),'b');%lactate
    % AA
...
for j=1:12
    ...
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0070',0.0039*v1(j),'b');%pyruvate
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0027',0.0124*v1(j),'b');%acetoin
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0002',1.8729*v1(j),'b');
    % AA
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0014',0.0163*v1(j),'b');%ala
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0015',-0.1,'l');%arg
    model_glu(j) = changeRxnBounds(model_glu(j),'EX0016',-0.1,'l');%cys
...
%% MORP
model_tre(1) = ecModel;
vmaxt=0.77;% maximum trehalose specific consumption rate (mmol/gDCW/h)
Kmt=10;% saturation constant (mmol/L) 
St=zeros(60,1);
St(1)=58.889;% trehalose concentration (mmol/L)
Slt=zeros(60,1);
Slt(1)=0;% lactate concentration (mmol/L)
dcwt=zeros(60,1);
dcwt(1)=1.85;% biomass (g/L)
v2=zeros(60,1);
